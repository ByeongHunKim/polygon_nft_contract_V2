...
contract NFT is ERC721Enumerable, Ownable {
    ...
    uint256 public per = 50;
    bool public paused = false;

    string public generalURI = "ipfs://QmUsjyVGzbSSrtdPYNFa61KAxDhrCV4ZgPqJbaVDWkcUXg/";
    string public uniqueURI = "ipfs://QmbBB2BtVVEavdFfdMgv3teiCiRDptptQ1nJgnRcYYkDmk/";
    ...
    function mint(address _to, uint256 _mintAmount) public payable 
    {
        ...
        uint256 randomNumber = getRandomNumber(_to, 100);
        uint256 tokenId = supply + _mintAmount;

        if(randomNumber <= per)
        {   
            setBaseURI(uniqueURI);
            emit result(tokenId, randomNumber, true);
        }else{
            setBaseURI(generalURI);
            emit result(tokenId, randomNumber, false);
        }
    ...
    function getRandomNumber(address _to, uint _range) internal returns (uint256)
    {
        ...
        uint256 randomNumber = randomNum % _range;
        return randomNumber;
    }
